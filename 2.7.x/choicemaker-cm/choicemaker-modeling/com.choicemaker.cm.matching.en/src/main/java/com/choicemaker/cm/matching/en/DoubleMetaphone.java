...
/**
 * Double metaphone (Philips 2000) is an improved version of metaphone. It works 
 * especially well for English. For certain strings that may have more than one 
...
public final class DoubleMetaphone {
	...
	private final static String[] EbEiElEpErEsEyIbIlInIe =
	...
	/**
	 * Creates a DoubleMetaphone instance that holds the primary and, if it 
	 * exists, alternate code.
	 * 
	 * @param txt the String
	 * @return the DoubleMetaphone encoding for txt
	 */
	public static DoubleMetaphone doubleMetaphone(String txt) {
		DoubleMetaphone res = new DoubleMetaphone();
		res.encode(txt);
		return res;
	}
	...
	/**
	 * Returns true if the primary codes agree.
	 * 
	 * @param om the object for comparison
	 * @return true if the primary codes are equal
	 */
	public boolean primaryEquals(DoubleMetaphone om) {
		return getPrimary().equals(om.getPrimary());
	}
	...
	public String getAlternate() {
		if (alternateS == null) {
			alternateS = alternate.toString();
		}
		return alternateS;
	}

	/**
	 * Returns true iff the primary and alternate codes are different.
	 * @return true iff the primary and alternate codes are different
	 */
	...
	private void encode(String in) {
		...
		while (primary.length() < encodeLimit || alternate.length() < encodeLimit) {
			...
			switch (input.charAt(current)) {
				...
				case ((char)128) : // "C-tail", ascii 128
					add('S');
					current++;
					// Note: no doublecheck
					break;
				...
				case 'C' :
					...
					if (input.regionMatches(current, "CH", 0, 2)) {
						...
						if (current == 0
							&& (stringAt(current + 1, 5, HaracHaris) || stringAt((current + 1), 3, HemHiaHorHym))
						...
						if ((stringAt(0, 4, Van_Von_)
							|| input.regionMatches(
								...
								"SCH ",
								0,
								3)) // 'architect' but not 'arch', 'orchestra', 'orchid'
				...
				case 'G' :
					...
					if ((current == 0)
						&& (input.charAt(current + 1) == 'Y' || stringAt(current + 1, 2, EbEiElEpErEsEyIbIlInIe))) {
						add('K', 'J');
						current += 2;
						break;
					}
					...
					// -ger-,  -gy-
					if ((input.regionMatches(current + 1, "ER", 0, 2) || input.charAt(current + 1) == 'Y')
				...
				case 'J' :
					...
					if (current == 0 && !stringAt(current, 4, "JOSE")) {
						...
						if (isVowel(current - 1)
							&& !isSlavoGermanic()
							&& ((input.charAt(current + 1) == 'A') || (input.charAt(current + 1) == 'O'))) {
							add('J', 'H');
						} else {
							if (current == last) {
								add('J', ' ');
				...
				case 'L' :
					if (input.charAt(current + 1) == 'L') {
						...
						if (((current == (length - 3)) && stringAt(current - 1, 4, AlleIllaIllo))
							|| ((stringAt((last - 1), 2, AsOs) || charAt(last, AO))
				...
				case 'M' :
					if ((stringAt(current - 1, 3, "UMB")
						&& (((current + 1) == last) || stringAt(current + 2, 2, "ER"))) //'dumb','thumb'
				...
				case ((char)209) : // "N-tilde", ascii 209
					current++;
					add('N');
					break;
				...
				case 'X' :
					//french e.g. breaux
					if (!(current == last
						&& (stringAt((current - 3), 3, EauIau) || stringAt((current - 2), 2, AuOu)))) {
						add('K');
						add('S');
					}
				...
				case 'Z' :
					...
					if (input.charAt(current + 1) == 'H') {
						...
						if (stringAt(current + 1, 2, ZaZiZo)
							|| (isSlavoGermanic() && (current > 0 && input.charAt(current - 1) != 'T'))) {
							alternate.append('T');
							add('S');
						} else {
							add('S');
		...
		input = null;
		different = !primary.equals(alternate);
	}
}
