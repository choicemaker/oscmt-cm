...
/**
 * A pair of objects, consisting of an input Map and a List of Maps,
 * each Map in the List representing a possible parse of the input.
 * @author rphall
 * @see AmbiguousParser
 */
public class MappedParsedInput {

	public final Map input;
	public final List output;

	public MappedParsedInput(Map input, List output) {
		this.input = input;
		this.output = output;
		checkThis(this);
	} // ctor(Map,List)

	public MappedParsedInput(String line) {
	...
	private static void checkThis(MappedParsedInput mpi) {
		if (mpi.input == null) {
		...
		for (int i = 0; i < mpi.output.size(); i++) {
			if (!(mpi.output.get(i) instanceof Map)) {
	...
	private static Object[] parseLine(String line) {
		...
		input = stripBraces(input);
		Map inputMap = parseMap(input);
		String list =
	 ...
	 * @throws IOException
	 */
	public static MappedParsedInput read(BufferedReader reader) throws IOException {
		if (reader == null) {
			throw new IllegalArgumentException("null reader");
		}
		MappedParsedInput retVal = null;
		String line = reader.readLine();
		if (line != null) {
			retVal = new MappedParsedInput(line);
		}
		return retVal;
	}
