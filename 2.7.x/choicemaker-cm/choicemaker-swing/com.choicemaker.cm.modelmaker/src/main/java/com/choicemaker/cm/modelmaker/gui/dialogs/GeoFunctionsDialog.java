...
/**
 * Dialog that demonstrates geo function calculations. It uses geo-entities
...
public class GeoFunctionsDialog extends JDialog {
	...
	// keep Enableness Guard as a separate object to be able to handle
	// differently changes in the different parts of the dialog
	...
	private JComboBox<String> geTypeBox1;
	private JComboBox<String> geTypeBox2;
	private DefaultComboBoxModel<String> geTypeBoxModel1 =
		new DefaultComboBoxModel<>();
	private DefaultComboBoxModel<String> geTypeBoxModel2 =
		new DefaultComboBoxModel<>();
	...
	private void changeEntity1Type() {
		...
		Vector<GeoMap.KeyField> fields = gm.getFields();
		GridBagConstraints c = new GridBagConstraints();
		c.insets = new Insets(4, 5, 5, 5);
		c.fill = GridBagConstraints.BOTH;
		...
		for (int n = 0; n < ent1CompNumb; n++) {
			if (n % 2 == 1)
				((JTextField) ent1Comp[n]).getDocument()
						.removeDocumentListener(dle);
		...
		if (gm.getFields() == null) {
			...
			for (int n = 0; n < fields.size(); n++) {
				...
				if (keyFiled.length >= 0) {
					...
					comp1.setDocument(new UpperCaseTextField.LimitDocument(
							keyFiled.length));
				} else
					comp1 = new UpperCaseTextField(15);
				entPanel1.add(comp1, c);
	...
	private void changeEntity2Type() {
		...
		Vector<KeyField> fields = gm.getFields();
		GridBagConstraints c = new GridBagConstraints();
		c.insets = new Insets(4, 5, 5, 5);
		c.fill = GridBagConstraints.BOTH;
		...
		for (int n = 0; n < ent2CompNumb; n++) {
			if (n % 2 == 1)
				((JTextField) ent2Comp[n]).getDocument()
						.removeDocumentListener(dle);
	...
	private void repopulateAvailableCollections() {
		...
		Iterator<String> gtRels =
			new TreeSet<String>(GeoHelper.getGeoTypes()).iterator();
		String typeName;
