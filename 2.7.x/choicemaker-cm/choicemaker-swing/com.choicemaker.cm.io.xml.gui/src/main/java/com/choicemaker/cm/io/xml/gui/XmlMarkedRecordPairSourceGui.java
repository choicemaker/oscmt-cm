...
/**
 * The MRPSGui associated the XmlMarkedRecordPairSource. An objects of this
 * class would be created by the XmlMarkedRecordPairSourceGuiFactory. It is used
 * by the AbstractApplication so that users can easily configure and build
 * XmlMarkedRecordPairSources.
 *
 * @author S. Yoakum-Stover
 * @author Martin Buechi
 */
public class XmlMarkedRecordPairSourceGui extends MarkedRecordPairSourceGui
		implements Enable {
	private static final long serialVersionUID = 1L;
	private static Logger logger = Logger
			.getLogger(XmlMarkedRecordPairSourceGui.class.getName());

	private static final MessageUtil m = ChoiceMakerCoreMessages.m;
	private static String FRAME_TITLE = m.formatMessage("io.xml.gui.label");
	...
	public XmlMarkedRecordPairSourceGui(ModelMaker parent,
	...
	public void buildContent() {
		...
		xmlFileNameLabel = new JLabel(m.formatMessage("io.xml.guilsource.file"));
	...
	protected void generate() {
		...
		try {
			...
			if (d == 1 && s == Integer.MAX_VALUE) {
				XmlMarkedRecordPairSink sink = new XmlMarkedRecordPairSink(
				...
				XmlMarkedRecordPairSinkFactory sinkFactory = new XmlMarkedRecordPairSinkFactory(
