...
/**
 * Represents a node in a suffix tree, specifically designed from elimination of
 * blocking sets.
 * 
 * This class is not thread-safe in the least.
 * 
 * Improvement: create a subclass of SuffixTreeNode for non-leaf nodes. Then can
 * move all the putChild, getChild, removeChild, etc. functionality (as well as
 * numKids and kids instance variables) in that class.
 * 
 * @author Adam Winkel
 */
...
public class SuffixTreeNode implements IIDSet {
	...
	/**
	 * We use an effective load factor as .75f, and use the the value of numKids
	 * * 100 as the threshold for expansion.
	 */
	...
	/**
	 * A pointer to this node's parent node. This will be null in the case of
	 * the root node or if this node has been removed from its parent.
	 */
	private final SuffixTreeNode parent;

	/**
	 * The next node in the linked list that this SuffixTreeNode is in its
	 * parent.
	 */
	private SuffixTreeNode next = null;

	/**
	 * The number of children of this SuffixTreeNode in the suffix tree. This
	 * should (at all times) be equal
	 */
	private int numKids = 0;

	/**
	 * Child buckets. Each bucket that contains a non-zero number of kids
	 * contains a linked list (linked by the SuffixTreeNode.next field) of child
	 * nodes.
	 */
	...
	public int getBlockingSetId() {
		throw new UnsupportedOperationException(
				"getBlockingSetId called on an internal node");
	}
	 ...
	 * PC 4/5/05
	 * 
	 * This returns a string representation of the tree. For example, it might
	 * look like
	 * 
	 * prefix 1 2 3 4 5 1 2 6 1 3 6 7 1 3 6 9
	 * 
	...
	/**
	 * This returns a string representation of the tree where each node is
	 * surrounded by [ and ]. For example: [2[4[6[8]][7]]]. Note that 4 has two
	 * children: 6 and 7.
	 * 
	 * @param sb
	 */
	...
	/**
	 * Creates and adds a child node with the specified ID to this
	 * SuffixTreeNode. If a child node with the specified childId exists, an
	 * IllegalArgumentException is thrown, as such a situation probably
	 * indicates a bug in the algorithm using the SuffixTreeNode class.
	 * 
	 * @throws IllegalArgumentException
	 *             if a node with childId already exists or if childId is an
	 *             invalid record ID (see Constructor)
	 */
	private SuffixTreeNode putChildImpl(SuffixTreeNode kid) {
		...
		while (node != null) {
			if (node.recordId == childId) {
				throw new IllegalArgumentException("Child node with id "
						+ childId + " already exists");
			}
			node = node.next;
		}
	...
	/**
	 * Ensures that this SuffixTreeNode has can add one more node without
	 * violating the hashtable/load factor contract. This contract says that a
	 * hash structure should obey the following formula: numKids / numBuckets <=
	 * LOAD_FACTOR.
	 * 
	 * This SuffixTreeNode's kids array will be grown up to and until the length
	 * is Integer.MAX_VALUE, at which point the performance will begin to
	 * degrade as hash buckets turn into long linked lists.
	 */
	...
	/**
	 * Expands the kids array to the specified length, and rehashes the child
	 * nodes to the new kids array.
	 * 
	 * Should only be called from ensureCapacity(), and only then if the kids
	 * array is non-null.
	 */
	...
	/**
	 * Removes a leaf node from it's parent node, and the parent node from its
	 * parent, until we run into a parent that has more than 1 child.
	 * 
	 * If this node doesn't have a parent, an IllegalStateException will be
	 * thrown. Similarly, if this node has some non-zero number of kids, an
	 * illegal
	 * 
	 */
	public void removeFromParentRecursive() {
		if (!isRemovableFromParent()) {
			throw new IllegalStateException(
					"This node is not removable from its parent!");
		}
	...
	/**
	 * Removes <code>child</code> from this node. If <code>child</code> is not a
	 * child of this node, bad things will happen.
	 * 
	 * Public methods calling this method (presently only
	 * removeFromParentRecursive) must ensure that this method is never called
	 * with an argument that is NOT a child of this node.
	 */
	 ...
	 * children. (If a node has children, it is subsumed by the blocking sets
	 * represented by each of its children, and their children, and their
	 * children...)
	 */
	private static class LeafSuffixTreeNode extends SuffixTreeNode {
		...
		public LeafSuffixTreeNode(SuffixTreeNode parent, long recordId,
				int blockingSetId) {
			super(parent, recordId);
			this.blockingSetId = blockingSetId;

			if (blockingSetId < 0) {
				throw new IllegalArgumentException("blockingSetId < 0: "
						+ blockingSetId);
			}
		}
