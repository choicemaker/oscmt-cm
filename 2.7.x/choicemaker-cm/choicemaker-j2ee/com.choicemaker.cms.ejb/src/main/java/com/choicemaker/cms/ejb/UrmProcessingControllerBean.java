 ...
 */
@Stateless
public class UrmProcessingControllerBean /* implements ProcessingController */{

	private static final Logger logger = Logger
			.getLogger(UrmProcessingControllerBean.class.getName());
	...
	static int deleteProcessingLogEntriesByJobId(EntityManager em, long id) {
		throw new Error("not yet implemented");
		// Query query =
		// em.createNamedQuery(QN_TRANSPROCESSING_DELETE_BY_JOBID);
	...
	static void updateStatusWithNotification(EntityManager em,
		...
		// logger.info(MessageBeanUtils.topicInfo("Sending", statusTopic,
		// data));
	...
	static boolean isOrderByDebuggingRequested() {
		if (_isOrderByDebuggingRequested == null) {
			String pn =
				ProcessingController.PN_PROCESSINGEVENT_ORDERBY_DEBUGGING;
