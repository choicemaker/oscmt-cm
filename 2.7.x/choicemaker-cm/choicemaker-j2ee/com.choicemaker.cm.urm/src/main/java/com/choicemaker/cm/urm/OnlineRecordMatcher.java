...
/**
 * Allows a client application to match specified records in online(synchronous)
 * mode.
 * <p>
 * Method <code>evaluatePair</code> main arguments are two records. It compares
 * them and evaluates their match probability and match decision.
 * <p>
 * Method <code>getMatchCandidates</code> main arguments are a (query) record
 * and a (master) records collection. It finds all records from the master
 * record collection that are matching (evaluated as MATCH or HOLD ) with the
 * query record.
 * <p>
 * The below example demonstrates how the <code>getMatchCandidates</code> would
 * be invoked within the client application.
 * <p>
 * As the query record it passes a record holder. A record holder is a class
 * that stores actual record data as values of it's variables. Record holders
 * are customer specific classes and generated from the customer matching schema
 * which is a part of the matching model. Customer specific record holder clases
 * are derived from the <code>IRecordHolder</code> interface that is derived
 * from <code>ISingleRecord</code>. Record holder are included into the
 * deployment package as part of the models.jar library. Another type of a
 * record that could be passed as the query is <code>GlobalRecordRef</code>. It
 * represents a reference to a record and doesn't contain actual data at
 * runtime.
 * <p>
 ...
 * the DB configuration name. DB Configuration is a set ChoiceMaker specific
 * views that implements the mapping from real customer tables into the
 * ChoiceMaker matching model record structure defined by model's schema file.
 * <p>
 ...
 * Object queryHomeRef =
 * 	initialContext.lookup(&quot;java:comp/env/ejb/OnlineRecordMatcher&quot;);
 * OnlineRecordMatcherHome qsh =
 * 	(OnlineRecordMatcherHome) PortableRemoteObject.narrow(queryHomeRef,
 * 			OnlineRecordMatcherHome.class);
 ...
 * 
 * EvaluatedRecord[] mc =
 * 	qs.getMatchCandidates(ph, masterCollection, modelName, differThreshold,
 * 			matchThreshold, maxNumMatches, resultFormat, trackingId);
 * 
 ...
 * 			+ &quot; First Name: &quot; + nb.getFirst_name() + &quot; Last Name: &quot;
 * 			+ nb.getLast_name() + &quot; Decision: &quot; + ms.getDecision()
 * 			+ &quot; Probability: &quot; + ms.getProbability() + &quot;\n&quot;);
 * }
 * </pre>
 * <p>
 * Each EvaluatedRecord in the returned array consists of a matched record and a
 * score reflecting the likelihood of the match. The format of the matched
 * records depends of the <code>resultFormat</code> passed to the
 * <code>getMatchCandidates</code>. In the above case record type is defined as
 * the <code>RecordType.HOLDER</code> and records will be returned as
 * PatientHolder objects (the same way they were passed). The score part of the
 * evaluated record contains the match probability (number between 0.0 and
 * 1.0))and the match decision (MATCH, DIFFER or HOLD). The decision is
 * calculated by comparing the match probability to the threshold probabilities
 * that are passed into the <code>getMatchCandidates</code>. Only records with
...
public interface OnlineRecordMatcher extends EJBObject {
	...
	/**
	 * Compares two records and evaluates their match probability and match
	 * decision based on the differThreshold, matchThreshold and the clues and
	 * rules of the matching model. The resulting match score is provided in the
	 * format defined by the <code>resultFormat</code> parameter.
	 * <p>
	 * 
	 * @param queryRecord
	...
	public MatchScore evaluatePair(ISingleRecord queryRecord,
			...
			ScoreType resultFormat, String externalId) throws ModelException,
			ArgumentException, RecordException, ConfigException,
			CmRuntimeException, RemoteException;

	/**
	 * Finds all records from the master record collection
	 * <code>masterCollection</code> that are evaluated as MATCH or HOLD with
	 * the query record <code>queryRecord</code>. Each evaluated record from the
	 * result array is a pair of a found record and a match score to the query
	 * record.
	 * <p>
	 * 
	 * @param queryRecord
