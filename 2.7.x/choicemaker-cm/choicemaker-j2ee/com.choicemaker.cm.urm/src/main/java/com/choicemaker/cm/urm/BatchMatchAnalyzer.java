...
/**
 * Allows a client application to match specified record collections and execute
 * further match result transitivity analysis in a batch (asynchronous) mode.
 * <p>
 * The below example demonstrates how the <code>startMatchAndAnalysis</code>
 * would be invoked within the client application.
 * <p>
 * 
 * <pre>
 * InitialContext initialContext = new InitialContext();
 * Object queryHomeRef =
 * 	initialContext.lookup(&quot;java:comp/env/ejb/BatchMatchAnalyzer&quot;);
 * BatchMatchAnalyzerHome qsh =
 * 	(BatchMatchAnalyzerHome) PortableRemoteObject.narrow(queryHomeRef,
 * 			BatchMatchAnalyzerHome.class);
 ...
 * boolean includeQueryRecord = true;
 * LinkCriteria lk = new LinkCriteria(GraphProperty.BCM, includeQueryRecord);
 * 
 * long l =
 * 	qs.startMatchAndAnalysis(qRc, null, &quot;OfflineMatch&quot;, diffTh, matchTh, 1000,
 * 			lk, AnalysisResultFormat.XML, trackingId);
 * 
 ...
 * </pre>
 * <p>
 * The result consists of connected record sets. Each connected record set
 * consists of linked record sets and single records. For more details see the
 * overview of the match result transitivity analysis in the API overview.
 * <p>
 * The result is serialized in the XML format. The other two possible formats
 * (H3L and R3L) are described in the <code>startMatchAndAnalysis</code> method
 * description.
 * 
 * @author emoussikaev
 */
public interface BatchMatchAnalyzer extends BatchBase {
	...
	/**
	 * As the first step starts record matching process (the same as
	 * <code>startMatch</code> call). After this step is completed divides
	 * records on connected record sets based on the match/hold connections
	 * between records. For each connected record set it applies the same
	 * analysis as online getCompositeMatchCandidates method identifying linked
	 * record sets. The result is serialized into a file according the
	 ...
	 * @param mRc
	 *            master record collection.
	 * @param modelName
	 *            name of the probability model.
	 ...
	 *            the format of the result serialization
	 * @param trackinglId
	...
	/**
	 * Works the same way as <code>startMatchAndAnalysis</code> method but at
	 * the first step uses the result of the record matching process with
	 * <code>jobId</code> identifier. The job with <code>jobId</code> should be
	 * completed otherwise his method throws an exception. The result is
	 * serialized into a file according the <code>serializationFormat</code>
	 * parameter.
	 * <p>
	 * 
	 * @param jobId
	...
	/**
	 * Returns an iterator that allows to traverse the result of the match
	 * analysis stored in a record collection <code>rc</code>. If format
	 * <code>s</code> is XML then the returned iterator is
	 * <code>ConntedRecordSetIter</code> . If format <code>s</code> is H3L or
	 * R3L then the returned iterator is <code>TripletIter</code>. If data in
	 * the record collection is not in the format compartible with
	 * <code>s</code> this method throws the RecordCollectionException.
	 * <p>
	...
	/**
	 * Returns an iterator that allows to traverse the result of the job with
	 * <code>jobId</code> identifier started by
	 * <code>startMatchAndAnalysis</code> (or<code>startAnalysis</code>) method.
	 * If format <code>s</code> is XML then the returned iterator is
	 * <code>ConntedRecordSetIter</code> . If format <code>s</code> is H3L or
	 * R3L then the returned iterator is <code>TripletIter</code>. If data in
	 * the record collection is not in the format compartible with
	 * <code>s</code> this method throws the RecordCollectionException.
