 ...
 *
 */
public class ComparisonTreeNode<T extends Comparable<T>> implements
		Serializable {
	...
	/**
	 * We hardcode the effective load factor as .75f, and use the the value of
	 * numKids * 100 as the threshold for expansion.
	 */
	...
	/**
	 * Returns true iff this ComparisonTreeNode has a valid blockingSetId. That
	 * is, this node represents the end of a blocking set.
	 */
	 ...
	 * PC 4/5/05
	 * 
	 * This returns a string represetation of the tree. For example, it might
	 * look like
	 * 
	 ...
	 * surrounded by [ and ]. For example: [S:2[S:4[S:6[M:8]][M:7]]]. Note that
	 * 4 has two children: 6 and 7.
	 * 
	 * @param sb
	 */
	...
	/*
	 * public String writeTree2 (String prefix) { StringBuffer sb = new
	 * StringBuffer (prefix); sb.append(Constants.OPEN_NODE); sb.append(type);
	 * sb.append(':'); sb.append(recordId.toString());
	 * 
	 * if (numKids > 0) { ArrayList children = getAllChildren(); for (int i=0;
	 * i<children.size(); i++) { ComparisonTreeNode kid = (ComparisonTreeNode)
	 * children.get(i); sb.append( kid.writeTree2("") ); } }
	 * 
	 ...
	 * ClassNotFoundException { char c = stream.readChar(); System.out.println
	 * (c); boolean stop = false; while (!stop) { if (c == Constants.OPEN_NODE)
	 * { c = stream.readChar(); this.type = c;
	 * 
	...
	public ComparisonTreeNode<T> putChild(T childId, char type,
			int blockingSetId) {
		return putChildImpl(new LeafComparisonTreeNode<T>(childId, type,
				blockingSetId));
	}
	 ...
	 * children...)
	 */
	private static class LeafComparisonTreeNode<T extends Comparable<T>>
			extends ComparisonTreeNode<T> {
		...
		// throw new
		// UnsupportedOperationException("Leaf nodes have no children");
		// }

		// public ComparisonTreeNode putChild(long childId) {
		// throw new
		// UnsupportedOperationException("Attempt to add a child to a leaf node");
		// }
