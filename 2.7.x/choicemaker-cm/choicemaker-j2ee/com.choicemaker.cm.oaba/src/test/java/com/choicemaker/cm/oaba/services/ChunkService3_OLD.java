...
@SuppressWarnings({
		"rawtypes", "unchecked" })
public class ChunkService3_OLD {
	...
	private static final Logger log =
		Logger.getLogger(ChunkService3_OLD.class.getName());

	protected static final String SOURCE = "ChunkService3_OLD";
	 ...
	 */
	public ChunkService3_OLD(IIDSetSource bSource, IIDSetSource osSource,
	...
	public void runService() throws BlockingException {
		...
		log.info("current processing event: "
				+ status.getCurrentProcessingEvent());
		if (status
	...
	private void createDataFiles() throws BlockingException {
		try {
			...
			if (numChunks <= maxFiles) {
				...
				createDataFiles(start, end, crSources, stageRecordSinks, 0, ind,
						stage, model);
	...
	private void createDataFiles(int start, int end,
		...
		for (int i = start; i < end; i++) {
			...
			if (crSources[i].hasNext()) {
				...
				log.severe(
						"missing record indices for chunk source[" + i + "]");
			}
	...
	private void createDataFile(RecordSource rs,
		...
		try {
			...
			context = "checking next record[" + count + "] from source ("
					+ rs.toString() + ")";
			while (rs.hasNext() && !stop) {
				context = "retrieving next record[" + count + "] from source ("
						+ rs.toString() + ")";
				Record r = rs.getNext();
	...
	public String toString() {
		return "ChunkService3_OLD [model=" + model + ", status=" + status
