...
import com.choicemaker.cm.oaba.impl.ComparablePairSink;
import com.choicemaker.cm.oaba.impl.ComparablePairSinkSourceFactory;
import com.choicemaker.cm.oaba.impl.ComparableSTSource;
import com.choicemaker.cm.oaba.impl.SimpleControl;

/**
 * This object takes in a source of SuffixTrees, creates pairs from the trees,
 * dedups the trees, and write the deduped pairs out to file.
 * 
 * It sets us the correct interfaces and relies on GenericDedpService to do the
 * work.
 * 
 * @author pcheung
 *
...
public class PairDedupService {
	...
	/**
	 * This constructor takes these inputs: 1. A suffix tree source 2. A factory
	 * to create temporary pair sources and sinks 3. An output pair sink to
	 * store the dedup pairs. 4. Maximum number of pairs to fit into memory at
	 * once.
	 * 
	 * @param source
	 * @param factory
	 ...
	 * @param max
	 */
	public PairDedupService(ISuffixTreeSource source,
			IPairIDSinkSourceFactory factory, IPairIDSink sink, int max) {
		this.source = source;
	...
	public void run() throws BlockingException {
		...
		ComparablePairSinkSourceFactory pFactory =
			new ComparablePairSinkSourceFactory(factory);
