...
public class MatchToBlockTransformer {
	...
	private static final Logger log = Logger
			.getLogger(MatchToBlockTransformer.class.getName());
	...
	/*
	 * This method goes through the MatchRecordSource to get a distinct list of
	...
	private void populateHashMaps() throws BlockingException {
		...
		while (mSource.hasNext()) {
			...
			if (!stageIDs.containsKey(mr.getRecordID1()))
				stageIDs.put(mr.getRecordID1(), null);

			if (mr.getRecord2Role() == RECORD_SOURCE_ROLE.STAGING) {
				if (!stageIDs.containsKey(mr.getRecordID2()))
					stageIDs.put(mr.getRecordID2(), null);
			} else {
				if (!masterIDs.containsKey(mr.getRecordID2()))
					masterIDs.put(mr.getRecordID2(), null);
			}
		}
	...
	/*
	 * This method uses the arrays in the translator to match the record IDs to
	 * internal IDs.
	 */
		...
		if (list != null) {
			...
			for (int i = 0; i < size; i++) {
				...
				if (masterIDs.containsKey(c))
					masterIDs.put(c, new Long(i));
			}
		}
	}

	/*
	 * This method reads the MatchRecord2Source, translates the record ids to
	 * internal ids, build equivalence classes with internal ids, adn write
	 * equivalence classes out as blocks.
	 */
		...
		try {
			...
			while (mSource.hasNext()) {
				...
				if (l1 == null) {
					...
					log.severe("l1 == l2 " + l1.toString() + " "
							+ l2.toString());
					log.severe("l1 == l2 " + mr.getRecordID1() + " "
							+ mr.getRecordID2());
				}
	 ...
	 */
	private void writeBlocks(EquivalenceClassBuilder builder)
	...
	/**
	 * This method creates blocks of internal ids from match result source of
	 * record ids.
	 *
