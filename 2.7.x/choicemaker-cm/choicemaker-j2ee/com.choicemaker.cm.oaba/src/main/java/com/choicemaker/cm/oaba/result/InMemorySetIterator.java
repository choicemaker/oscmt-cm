...
public class InMemorySetIterator {
	...
	private static final Logger log = Logger
			.getLogger(InMemorySetIterator.class.getName());
	...
	/**
	 * This constructor takes in an int array of roots and builds sets with the
	 * same root. The input is SetJoiner.flatten's output.
	 * 
	 * @param roots
	 */
	...
	private IntArrayList readNext() {
		...
		if (position >= size)
			throw new IllegalStateException(
					"Invalid root array.  The final set only has one element.");
		...
		if (list.size() == 1)
			throw new IllegalStateException(
					"Invalid root array.  A set only has one element.");
