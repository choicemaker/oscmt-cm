...
/**
 * This object takes in a source that gets Comparable object and dedups that
 * source. It does the following:
 * 
 * 1. Put Comparables on a TreeSet to filter out dupes. 2. If the tree gets
 * full, write to a temp file. The content of the temp file is sorted. 3. Merge
 * all the temp files by opening all of them and walk through them.
 * 
 * @author pcheung
 *
 */
...
public class GenericDedupService {
	...
	/**
	 * This reads from cSource and put Comparables on a TreeSet and when the set
	 * gets too big, it dumps it to a temp file.
	 * 
	 *
	 */
	private void writeToFiles() throws BlockingException {
		...
		log.fine("Reading from " + cSource.getInfo() + " writing to "
				+ tempSink.getInfo());
		...
		while (cSource.hasNext() && !stop) {
			...
			if (!matches.contains(c)) {
				...
				if (numBefore % INTERVAL == 0
						&& isFull(matches.size(), maxMatches)) {
	 ...
	 * 
	 * @param tempSinks
	 *            - ArrayList of temp files
	 * @param cSink
	 *            - the output
	 * @param cFactory
	 *            - IComparableSinkSourceFactory for moving other utility
	 *            functions.
	 * @param delete
	...
	private static int count(IComparableSink snk,
			IComparableSinkSourceFactory cFactory) throws BlockingException {
	...
	public static int nonOverlap(IComparableSource source,
		...
		while (source.hasNext()) {
			...
			if (nextComp != null) {
				// get the next dup that is at least as great as the current
				// object.
	...
	/*
	 * This gets the next Comparable from the source. If no more, then it
	 * returns null. It assumes that source is already open.
	 */
	private static Comparable getNextComp(IComparableSource dups)
			throws BlockingException {
	 ...
	 */
	private static Comparable getNextCompBiggerThan(IComparableSource dups,
			Comparable c) throws BlockingException {
		Comparable nextComp = null;
		boolean stop = false;
