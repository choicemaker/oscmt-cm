...
public class ChunkService2 {
	...
	// list of block sinks, the ith comparisonSinks correspond to the ith
	// recSinks
	 ...
	 * @param cFactory
	 *            - factory to store comparison groups
	 * @param maxChunkSize
	 *            - maximum size of a chunk
	 * @param status
	 *            - status of the system
	 */
	public ChunkService2(IBlockSource bSource, IBlockSource osSource,
			...
			IComparisonArraySinkSourceFactory cFactory, int maxChunkSize,
	...
	public void runService() throws XmlConfException, BlockingException {
		...
		if (status.getCurrentProcessingEventId() == OabaProcessing.EVT_DONE_CREATE_CHUNK_DATA) {
			...
			status.setCurrentProcessingEvent(OabaProcessingEvent.DONE_CREATE_CHUNK_IDS,
					Integer.toString(comparisonSinks.size()));

			createDataFiles();

		} else if (status.getCurrentProcessingEventId() == OabaProcessing.EVT_DONE_CREATE_CHUNK_IDS) {
			// create the chunk data files
			int numFiles = Integer.parseInt(status.getCurrentProcessingEventInfo());
			log.info("Recovering from creating chunk data, numFiles "
					+ numFiles);
	...
	private void createDataFiles() throws BlockingException, XmlConfException {
		...
		try {
			...
			if (master != null) {
				...
				createDataFile(master, masterModel, ind, crSources,
						recordSinks, translator.getSplitIndex());
			}
	...
	/**
	 * This method creates the chunk data files from the chunk id files for the
	 * given record source.
	 * 
	 *
	 */
	 ...
	 * @throws IOException
	 */
	private void createIDs(IBlockSource source, boolean isOS, int skip)
		...
		while (source.hasNext()) {
			...
			if (rows.size() > maxChunkSize) {
				...
				// if (countAll > 2000) throw new RuntimeException
				// ("failed test");
			}
