 ...
 * 
 *
 *             Represents a node in a prefix tree, specifically designed from
 *             elimination of blocking sets.
 * 
 *             A Prefix Tree may looks like (2, 5, 7, 9, (10, 15)), where commas
 *             separate the levels. This tree represents two sets: (2, 5, 7, 9,
 *             10) and (2, 5, 7, 9, 15).
 * 
 *             Note that the tree is ordered. Node 9 has two children. Nodes 10
 *             and 15 are childless.
 * 
 * 
 *             This class is not thread-safe in the least.
 * 
 *             TODO: create a subclass of SuffixTreeNode for non-leaf nodes.
 *             Then can move all the putChild, getChild, removeChild, etc.
 *             funcationality (as well as numKids and kids instance variables)
 *             in that class.
 * 
...
public class PrefixTreeNode {
	...
	/**
	 * The number of children of this PrefixTreeNode in the prefix tree. This
	...
	/**
	 * Returns true iff this PrefixTreeNode represents the end of a blocking
	 * set.
	 */
	...
	public PrefixTreeNode getChild(long childId) {
		if (kids == null) {
			...
			PrefixTreeNodeComparator comparator =
				new PrefixTreeNodeComparator();
	 ...
	 * PrefixTreeNode. If a child node with the specified childId exists, an
	 ...
	 * 
	 * If the child has an id than the parent, throw an IllegalArgumentException
	 * because there is a flaw in the logic of the code.
	 * 
	...
	private PrefixTreeNode putChildImpl(PrefixTreeNode kid) {
		...
		if (childId < recordId) {
			throw new IllegalArgumentException("Child node with id " + childId
					+ " is less than " + "current id " + recordId);
		}
		...
		if (kids == null) {
			...
			// Should not try to insert a child node with the same id as an
			// existing node.
	...
	private static class PrefixTreeNodeComparator implements Comparator {
		...
		public int compare(Object o1, Object o2) {
			...
			if (p1.getRecordId() < p2.getRecordId())
				return -1;
			else if (p1.getRecordId() > p2.getRecordId())
				return 1;
			else
				return 0;
		}
