 ...
 * 
 *         If this service processes first 10 files for dedup and then it is
 *         stopped, when it starts again for recovery, it won't reload the first
 *         10 files. It'll start with file 11. If block A in file 1 is a subset
...
public class BlockDedupService {
	...
	private static final Logger log = Logger.getLogger(BlockDedupService.class
			.getName());
	...
	private void startDedup(int startPoint) throws BlockingException {
		...
		for (int i = startPoint; i < parts.length; i++) {
			...
			if (source.exists()) {
				...
				// System.out.println (i + " Processing: " + source.getInfo() +
				// " count: " + numBlocksIn);
