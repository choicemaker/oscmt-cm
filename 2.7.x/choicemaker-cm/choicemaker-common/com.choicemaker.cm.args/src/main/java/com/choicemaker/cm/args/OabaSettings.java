 ...
 * configuration. However, since these settings rarely change, and are often
 * reused between jobs, it makes more sense to store them separately from any
 * particular model. In a J2EE environment, OABA settings are cached in memory
...
public interface OabaSettings extends AbaSettings {
	...
	boolean DEFAULT_USE_DATABASE_RECORD_ID_TRANSLATIONS = false;

	/**
	 * @return The maximum size of a 'regular' OABA blocking set. Similar to the
	 *         ABA {@link #getLimitPerBlockingSet() blocking set limit}.
	 */
	int getMaxBlockSize();
	...
	/**
	 * The threshold number of records beyond which query records are matched in
	 * a batch. Below this threshold, query records are matched to the reference
	 * records one at a time. Usually set to zero (0) so that batch processing
	 * is always performed, but should be set to a value larger than the number
	 * of query records if there are only a few thousand query record and the
	 * number of reference records is a few million or more.
	 * 
	...
	/**
	 * To save memory, record ids are translated to int-valued aliases when
	 * blocking values are computed. The most general method is to compute
	 * translations on the fly, as records are retrieved from a data source.
	 * This is the default method used by the OABA service. A considerable
	 * savings in time can be achieved, however, when the record source is a
	 * relational database and a <code>DbRecordIdTranslatorFactory</code> has
	 * been configured. This flag indicates whether database translations are
	 * available and should be used by the OABA service. (This flag is
	 * irrelevant to the Transitivity service.)
	 * 
	 * @return true if database translations are available and should be used by
	 *         the OABA service when blocking values are computed.
	 */
	boolean useDatabaseRecordIdTranslations();
