...
public class Translator25b extends Translator25 implements TargetTags, Modifiers, ITranslator {
	...
	public Translator25b(ITargetPrinter targetPrinter, ICompilationUnit unit) throws CompilerException {
	...
	*/
	...
	public void visit(ClueDecl t) throws CompilerException { // separate procedure only needed if referenced elsewhere
		...
		//temporary disable CSE elimination
		Tree[] then = { new Assign(evalNumIdent, eval_num_ident(t.pos)), new Assign(exprIdent, t.expr)};
		//TreeList ts = CSE(new Assign(exprIdent, t.expr), new NamePool());
		//TreeList thencode = new TreeList();
		//thencode.append(new Assign(evalNumIdent, eval_num_ident(t.pos)));
	...
	/*
	public void printTree(String title,Tree t)
	{
		
		try{
			System.out.print("\n");
			System.out.print(title+"\n");
			new TargetPrinter25().printExpr(t).flush();
		}
		catch(Exception e){
			e.printStackTrace();	
		}
	...
	*/
	...
	public boolean visitAorE(Quantified t) throws CompilerException {
		...
		for (int i = 0; i < t.vars.length; i++)
			vars.add(t.vars[i]);
		TreeList dclFactors = new TreeList();	
		TreeList[] factors = new TreeList[t.vars.length];
		for (int i = 0; i < t.vars.length - 1; i++) {
			vars.remove(t.vars[i]);
			Factorizer1 f = new Factorizer1(unit, vars,  pool);
			//printTree("Original clue:",res);
			res.apply(f);
			if (f.res != null)
				res = f.res;
			factors[i] = f.initLocal;
			dclFactors.append( f.factorDecls.toArray());
			if(f.fres != null)    {  //printTree("Precondition",f.fres);
				Convertor32 posConv = new Convertor32(unit,true,t.quantifier != EXISTS);
				Convertor32 negConv = new Convertor32(unit,false,t.quantifier != EXISTS);
				Tree retb = new Return (new Literal(t.pos, BOOLEAN, t.quantifier == EXISTS?Boolean.TRUE:Boolean.FALSE));
				f.fres.apply(posConv);
				if(posConv.resExp == null)	{ //System.out.print("\nPositive\n"+posConv.resConst);
					if(posConv.resConst)
						throw new CompilerException(t.quantifier == EXISTS?"Exists":"All"+" condition is logically true statement."); //factors[i].append(retb);
				}
				else	{ //printTree("\nPositive\n",posConv.resExp); 
					factors[i].append(new Cond(posConv.resExp, retb, null));
				}
				f.fres.apply(negConv);
				if(negConv.resExp == null)	{ //System.out.print("\nNegative\n"+negConv.resConst);
					if(!negConv.resConst)
						throw new CompilerException(t.quantifier == EXISTS?"Exists":"All"+" condition is logically false statement.");//factors[i].append(new Continue(""));
				}
				else	{ //printTree("\nNegative\n",negConv.resExp);
					factors[i].append(new Cond( new Unop(t.pos, NOT, negConv.resExp), new Continue(""), null));
				}
			}					
			vars.add(t.vars[i]);
		}
		...
		Tree[] bl1 = new Tree[1];
		bl1[0] = new Return(new Literal(t.pos, BOOLEAN, (t.quantifier == EXISTS)?Boolean.TRUE:Boolean.FALSE));  							

		Tree inner = (t.quantifier == EXISTS) ?
						new Cond(res, new Block(bl1), null ) : //new Block(bl1)
						new Cond(new Unop(t.pos, NOT, res),new Block(bl1), null);
		...
		// unalias variables
		//inner = unalias(inner);
		aBody.append(dclFactors.toArray());
		aBody.append(inner);
		aBody.append(new Return(new Literal(t.pos, BOOLEAN, Boolean.valueOf(t.quantifier == ALL))));
...
//	Tree[] bl = new Tree[3];
//	Tree[] cArgsP = { new Binop( Location.NOPOS, PLUS, new Literal(t.pos, STRING, "Positive fired >>"),new Ident(t.pos, "precFired"))  };
//	bl[0] = new Assign( new Ident(t.pos, "precFired"), new Binop(Location.NOPOS, PLUS, new Ident(t.pos, "precFired"), one_lit(Location.NOPOS)));
//	bl[1] = new Apply(t.pos, cat_error(t.pos), cArgsP);
//	bl[2] = retb; //new Block(bl) 							

//	Tree[] bl = new Tree[3];
//	Tree[] cArgsP = { new Binop( Location.NOPOS, PLUS, new Literal(t.pos, STRING, "Negative fired >>"),new Ident(t.pos, "precFired"))  };
//	bl[0] = new Assign( new Ident(t.pos, "precFired"), new Binop(Location.NOPOS, PLUS, new Ident(t.pos, "precFired"), one_lit(Location.NOPOS)));
//	bl[1] = new Apply(t.pos, cat_error(t.pos), cArgsP);
//	bl[2] = new Continue(""); 							
...
//	Tree[] bl = new Tree[2];
//	Tree[] bl1 = new Tree[1];
//	Tree[] cArgsP = { new Binop( Location.NOPOS, PLUS, new Literal(t.pos, STRING, "Instep >>"),new Ident(t.pos, "inStep"))  };
//	bl[0] = new Assign( new Ident(t.pos, "inStep"), new Binop(Location.NOPOS, PLUS, new Ident(t.pos, "inStep"), one_lit(Location.NOPOS)));
//	bl[1] = new Apply(t.pos, cat_error(t.pos), cArgsP);
