...
import java.util.*;
public class FrozenImpl____descriptor implements com.choicemaker.cm.core.Descriptor {
public static com.choicemaker.cm.core.Descriptor instance = new FrozenImpl____descriptor();
...
int cur = 0;
if(cur + r0.frozen.length <= row) {
cur += r0.frozen.length;
} else {
switch(col) {
case 0:
return r0.frozen[row-cur].src_system_id;
default:
...
switch(col) {
case 0:
r0.frozen[row-cur].src_system_id = value;
break;
default:
throw new IndexOutOfBoundsException();
}
...
switch(col) {
case 0:
return r0.frozen[row-cur].__v_src_system_id;
default:
throw new IndexOutOfBoundsException();
}
}
...
int cur = 0;
if(cur + r0.frozen.length < row || (above && cur + r0.frozen.length == row && row != 0)) {
cur += r0.frozen.length;
} else {
FrozenImpl[] tmp = new FrozenImpl[r0.frozen.length + 1];
System.arraycopy(r0.frozen, 0, tmp, 0, (row - cur));
tmp[(row - cur)] = FrozenImpl.instance();
System.arraycopy(r0.frozen, (row - cur), tmp, (row - cur) + 1, tmp.length - 1 - (row - cur));
r0.frozen = tmp;
return;}
throw new IndexOutOfBoundsException();
}
...
FrozenImpl[] tmp = new FrozenImpl[r0.frozen.length - 1];
System.arraycopy(r0.frozen, 0, tmp, 0, (row - cur));
System.arraycopy(r0.frozen, (row - cur) + 1, tmp, (row - cur), tmp.length - (row - cur));
r0.frozen = tmp;
return;}
throw new IndexOutOfBoundsException();
}
