...
import java.util.*;
public class IndexAndOutstandingImpl____descriptor implements com.choicemaker.cm.core.Descriptor {
public static com.choicemaker.cm.core.Descriptor instance = new IndexAndOutstandingImpl____descriptor();
...
int cur = 0;
if(cur + r0.indexAndOutstanding.length <= row) {
cur += r0.indexAndOutstanding.length;
} else {
switch(col) {
case 0:
return r0.indexAndOutstanding[row-cur].src_system_id;
case 1:
return String.valueOf(r0.indexAndOutstanding[row-cur].patient_id);
default:
...
return r0.indexAndOutstanding[row-cur].src_system_id;
case 1:
return new Integer(r0.indexAndOutstanding[row-cur].patient_id);
default:
throw new IndexOutOfBoundsException();
}
}
...
return IndexAndOutstandingImpl.class;
}
...
switch(col) {
case 0:
r0.indexAndOutstanding[row-cur].src_system_id = value;
break;
case 1:
r0.indexAndOutstanding[row-cur].patient_id = value.length() == 0 ? (int)0 : Integer.parseInt(value);
break;
default:
throw new IndexOutOfBoundsException();
}
...
switch(col) {
case 0:
return r0.indexAndOutstanding[row-cur].__v_src_system_id;
case 1:
return r0.indexAndOutstanding[row-cur].__v_patient_id;
default:
throw new IndexOutOfBoundsException();
}
}
...
int cur = 0;
if(cur + r0.indexAndOutstanding.length < row || (above && cur + r0.indexAndOutstanding.length == row && row != 0)) {
cur += r0.indexAndOutstanding.length;
} else {
IndexAndOutstandingImpl[] tmp = new IndexAndOutstandingImpl[r0.indexAndOutstanding.length + 1];
System.arraycopy(r0.indexAndOutstanding, 0, tmp, 0, (row - cur));
tmp[(row - cur)] = IndexAndOutstandingImpl.instance();
System.arraycopy(r0.indexAndOutstanding, (row - cur), tmp, (row - cur) + 1, tmp.length - 1 - (row - cur));
r0.indexAndOutstanding = tmp;
return;}
throw new IndexOutOfBoundsException();
}
...
IndexAndOutstandingImpl[] tmp = new IndexAndOutstandingImpl[r0.indexAndOutstanding.length - 1];
System.arraycopy(r0.indexAndOutstanding, 0, tmp, 0, (row - cur));
System.arraycopy(r0.indexAndOutstanding, (row - cur) + 1, tmp, (row - cur), tmp.length - (row - cur));
r0.indexAndOutstanding = tmp;
return;}
throw new IndexOutOfBoundsException();
}
...
PatientImpl r0 = (PatientImpl)ri;
int num = 0;
num += r0.indexAndOutstanding.length;
return num;
}
