...
private static Logger logger = Logger.getLogger(com.choicemaker.cm.custom.mci.gend.internal.MciRecords.IdsImpl.class.getName());
...
try {
__v_patient_id = StringUtils.nonEmptyString(patient_id) && !Sets.includes("invalidPatientIds", patient_id) && (!(FacilityUtils.isMedicalRecord(identifier_type) || FacilityUtils.isFacility(identifier_type)) || FacilityUtils.noIdenticalVital(patient_id, outer));
if(__src1.includes(__src)) {
if(__v_patient_id) {
patient_id_norm = FacilityUtils.removeNonLettersNonDigitsAndLeadingZeros(patient_id);
