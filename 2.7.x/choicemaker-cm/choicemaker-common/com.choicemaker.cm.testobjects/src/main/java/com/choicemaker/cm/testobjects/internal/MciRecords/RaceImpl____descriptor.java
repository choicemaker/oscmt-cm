...
import java.util.*;
public class RaceImpl____descriptor implements com.choicemaker.cm.core.Descriptor {
public static com.choicemaker.cm.core.Descriptor instance = new RaceImpl____descriptor();
private static HashMap m;
private static ColumnDefinition[] cols = {
new ColumnDefinition("race_cd", "race_cd", 100, JLabel.CENTER)};
...
int cur = 0;
if(cur + r0.race.length <= row) {
cur += r0.race.length;
} else {
switch(col) {
case 0:
return r0.race[row-cur].race_cd;
default:
...
switch(col) {
case 0:
return r0.race[row-cur].__v_race_cd;
default:
throw new IndexOutOfBoundsException();
}
}
...
int cur = 0;
if(cur + r0.race.length < row || (above && cur + r0.race.length == row && row != 0)) {
cur += r0.race.length;
} else {
RaceImpl[] tmp = new RaceImpl[r0.race.length + 1];
System.arraycopy(r0.race, 0, tmp, 0, (row - cur));
tmp[(row - cur)] = RaceImpl.instance();
System.arraycopy(r0.race, (row - cur), tmp, (row - cur) + 1, tmp.length - 1 - (row - cur));
r0.race = tmp;
return;}
throw new IndexOutOfBoundsException();
}
...
RaceImpl[] tmp = new RaceImpl[r0.race.length - 1];
System.arraycopy(r0.race, 0, tmp, 0, (row - cur));
System.arraycopy(r0.race, (row - cur) + 1, tmp, (row - cur), tmp.length - (row - cur));
r0.race = tmp;
return;}
throw new IndexOutOfBoundsException();
}
