 ...
 * </p>
 */
public interface CMExtensionPoint {
	/**
	 * Returns all configuration elements from all extensions configured into
	 * this extension point. Returns an empty array if this extension point has
	 * no extensions configured, or none of the extensions contain configuration
	 * elements.
	 *
	 ...
	 * Since an extension might not have an identifier, some extensions can only
	 * be found via the <code>getExtensions</code> method.
	 *
	 * @param extensionId
	 *            the unique identifier of an extension (e.g.
	...
	/**
	 * Returns all extensions configured into this extension point. Returns an
	 * empty array if this extension point has no extensions.
	 *
	 ...
	 * returned as a URL path relative to the plug-in installation URL. Returns
	 * the empty string if no schema for this extension point is specified in
	 ...
	 * a non-empty string containing no period characters (<code>'.'</code>) and
	 * is guaranteed to be unique within the defining plug-in.
	 *
