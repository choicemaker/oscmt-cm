...
public interface CMPlugin {
	...
	// // strip off the jar separator at the end of the url then do a recursive
	// call
	// // to interpret the sub URL.
	// String file = target.getFile();
	// file = file.substring(0, file.length() -
	// PlatformURLHandler.JAR_SEPARATOR.length());
	// try {
	...
	// * @return an input stream
	// */
	// public final InputStream openStream(CMPath file, boolean localized)
	// throws IOException {
	// URL target = PlatformURLFactory.createURL(getDescriptor().getInstallURL()
	// + file.toString());
	 ...
	 *
	 * @param value
	 *            whether or not this plugi-in is in debug mode
	 */
	 ...
	 * </p>
	 * <p>
	 * Plug-in shutdown code should be robust. In particular, this method should
