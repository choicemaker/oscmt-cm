...
public class PluginParser extends DefaultHandler implements IModel {
			...
			case FRAGMENT_STATE :
				if (elementName.equals(PLUGIN) || elementName.equals(FRAGMENT)) {
					...
					if (extPointVector.size() > 0) {
						root.setDeclaredExtensionPoints(
							(ExtensionPointModel[]) extPointVector.toArray(
								new ExtensionPointModel[extPointVector.size()]));
					...
					if (extVector.size() > 0) {
						root.setDeclaredExtensions(
							(ExtensionModel[]) extVector.toArray(new ExtensionModel[extVector.size()]));
			...
			case PLUGIN_REQUIRES_STATE :
				if (elementName.equals(PLUGIN_REQUIRES)) {
					...
					if (importVector.size() > 0) {
						...
						parentDescriptor.setRequires(
							(PluginPrerequisiteModel[]) importVector.toArray(
								new PluginPrerequisiteModel[importVector.size()]));
					}
				}
				break;
	...
	public void handlePluginState(String elementName, Attributes attributes) {
		...
		if (elementName.equals(RUNTIME)) {
			...
			if (((whatIsIt instanceof PluginDescriptorModel)
				&& (((PluginDescriptorModel) objectStack.peek()).getRuntime() != null))
				|| ((whatIsIt instanceof PluginFragmentModel)
					&& (((PluginFragmentModel) objectStack.peek()).getRuntime() != null))) {
	...
	public void parseConfigurationElementAttributes(Attributes attributes) {
		...
		parentConfigurationElement.setProperties(
			(ConfigurationPropertyModel[]) propVector.toArray(new ConfigurationPropertyModel[propVector.size()]));
		propVector = null;
	}
		...
		for (int i = 0; i < len; i++) {
			...
			if (attrName.equals(EXTENSION_NAME))
				...
				if (attrValue.lastIndexOf('.') == -1) {
					String baseId =
						parent instanceof PluginDescriptorModel
							? parent.getId()
							: ((PluginFragmentModel) parent).getPlugin();
		...
		for (int i = 0; i < len; i++) {
			...
			if (attrName.equals(PLUGIN_REQUIRES_PLUGIN))
				...
				if (PLUGIN_REQUIRES_MATCH_PERFECT.equals(attrValue))
					...
					(PLUGIN_REQUIRES_MATCH_EQUIVALENT.equals(attrValue))
